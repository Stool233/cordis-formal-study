...
| 使用流程 | 资产 | SHA-256 前缀 |
| --- | --- | --- |
| 历史 baseline、conformance、core/full、nightly、release | 2026-08-11 构建的 TLC | `ab323b79802a` |
| 当前上游对齐 | 2026-09-04 构建的 TLC | `b658b4e504fd` |
...
门户把这些设置同时传递给 Harness 子进程与直接调用的 Cordis 命令。Conformance、Upstream alignment、Nightly、Release 共用同一个解析器。绕开门户、直接调用旧 fork 的 runner 时，仍需设置上述变量，才能避开其原始下载路径。
...
`npm test` 覆盖锁选择、文件缺失或损坏、错误覆盖与 manifest 边界。`test:tools` 需要 Java 21：它在真实 kit 进程中禁止 fetch，将旧缓存文件名写入错误内容，分别使用两套固定工具解析五个模块，并运行一个两状态 TLC 冒烟模型。两条形式化 CI 都在复现前运行该检查。`verify` 校验每份入库资产的哈希，并确认两份版本锁均有对应工具。
